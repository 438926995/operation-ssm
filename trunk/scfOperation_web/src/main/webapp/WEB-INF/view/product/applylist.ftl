                ...
                <div class="box-header">
					...
					<form action="" method="get" id="queryUser">
						<div id="user_list_search"  >
							...
							<@layout.security.authorize url="product/addApplyView">
								<input type="button" value="添加申请条件" onclick = "javascript:goApplyAdd()"  >
                  ...
                  <table id="user_list" class="table table-bordered table-hover">
                    ...
                    <#list tbData.list as detail>
						<tr>
							...
							<td style="text-align:left">${detail.ruleContent!""}</td>
							<td style="text-align:left">
							<@layout.security.authorize url="product/editApplyView">
								<a href="${basePath}/product/editApplyView/${detail.prId!!}" >编辑</a>
							</@layout.security.authorize>
							<@layout.security.authorize url="product/applyDel">
								<a href="javascript:deleteProductRuleInfo('${detail.prId!!}')">删除</a>
							</@layout.security.authorize>
							</td>
						</tr>
					</#list>
...
<script>
function deleteProductRuleInfo(prId){
	if(confirm("确定要删除数据吗")){
		$.ajax({
		   	type: "GET",
		   	...
		   	success: function(resultObj){
		   		if(resultObj.isSuccess){
		   			alertMessageBox("融资产品申请条件",resultObj.message,function(){
		   					window.parent.location.reload();
		   				});
				}else{
					alertMessageBox("融资产品申请条件",resultObj.message,function(){window.parent.location.reload();});
				}
		   	},
		   	error:function(resultObj){  
		   	 	alertMessageBox("融资产品申请条件","出现异常，请稍后重试！");
		   	}
		});
	}
}
...
function goApplyAdd(){
  	window.location.href="${basePath}/product/addApplyView/${fpId}";
}
