...
<div class="content-wrapper">
	...
	<div class="box box-primary">
        <div class="box-header with-border">
          <h3 class="box-title">审批节点添加</h3>
        </div><!-- /.box-header -->
        <!-- form start -->
        <form role="form" id="apvTeam" action="${basePath}/flowmanage/addNodeSave" method="post" data-parsley-validate="" enctype="multipart/form-data">
          <div class="box-body"> 
          
           <input type="hidden" class="form-control" id="procsID" name="procsID" value="${afp.procsID!}">
           
           <input type="hidden" class="form-control" id="foID" name="foID" value="${afp.foID!}">
         ...
         <#--org end-->
           	...
           	<input type="hidden" class="form-control" id="teamID" name="teamID" value="${(afn.teamID)!}">
			<input type="hidden" class="form-control" id="appUserIDs" name="appUserIDs" value="${(afn.appUserIDs)!}">
            <input type="text" class="form-control" id="appUserNames" name="appUserNames" value="${(afn.appUserNames)!}" readonly="readonly"  style="margin:5px;color:blue;display:none" >
...
<script>
	...
	function typeChoose(){
		var type = $("#type").val();
		if(type=="") {
			$("#typeEmpty").addClass("has-error");
 			$("#typeEmpty").html('<font color="red"><strong>节点类型不能为空！</strong></font>');
 			$("#typeEmpty").show();
	        return false;
		}
		<#-- 判断是否是运营人员 -->
